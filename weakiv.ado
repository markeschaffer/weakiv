...
* Clean command line of extraneous weight [] and option ,
	...
	local cmd `1'
	local legalcmd	"ivregress ivreg2 ivreg2h xtivreg xtivreg2 ivprobit ivtobit xtabond2 ddml"
...
* Need to pass `wvar' so that values saved by xtabond2 in e(wt) can be assigned to it
	...
	if "`e(cmd)'"=="ddml" {
		parse_ddml, touse(`touse') strong(`strong') subset(`subset') testexog(`testexog')
	}
...
program define parse_ddml, sclass
	version 11.2
	syntax	[ ,								///
				touse(varname)				///
				strong(varlist fv ts)		///
